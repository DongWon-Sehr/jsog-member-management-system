<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <!-- Activity Member Card - Clickable to navigate to Members tab -->
      <div 
        @click="currentView = 'Members'"
        class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4 cursor-pointer hover:border-blue-200 hover:shadow-md transition-all active:scale-95 group"
      >
        ...
        <div>
          <p class="text-sm font-medium text-gray-500">활동 회원</p>
          <p class="text-2xl font-bold text-gray-900">{{ dashboardSummary.activeMemberCount }}명</p>
        </div>
      </div>
      ...
      >
        ...
        <div>
          <p class="text-sm font-medium text-gray-500">이번 주 인증</p>
          <p class="text-2xl font-bold text-gray-900">{{ dashboardSummary.weeklyWorkoutCount }}회</p>
        </div>
      </div>
      ...
      <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
        ...
        <div>
          <p class="text-sm font-medium text-gray-500">누적 리워드</p>
          <p class="text-2xl font-bold text-gray-900">{{ (dashboardSummary.totalPrizeAmount || 0).toLocaleString() }}원</p>
        </div>
      </div>
    </div>
