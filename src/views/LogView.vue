  ...
  <div class="space-y-0 pb-6">
    ...
    <div class="sticky top-16 z-30 bg-white">
      ...
      <div class="bg-gray-50/95 backdrop-blur-sm pt-6 px-6 pb-3 border-x border-gray-100">
        <div class="hidden md:grid grid-cols-12 gap-4 px-8 py-3 bg-gray-100 rounded-xl text-[11px] font-black text-gray-500 uppercase tracking-widest shadow-sm border border-gray-200">
    ...
    <div class="bg-gray-50/50 border-x border-b border-gray-100 rounded-b-3xl overflow-hidden min-h-[500px]">
      <div v-if="isLoading && logs.length === 0" class="py-32 flex flex-col items-center justify-center text-center">
      ...
      <div v-else class="px-6 pb-6 space-y-3 pt-1">
        ...
        <!-- Empty State -->
        <div v-if="!isLoading && logs.length === 0" class="py-32 text-center">
          ...
          <p class="text-gray-400 font-black text-lg">기록된 로그가 없습니다.</p>
        </div>
      </div>
    </div>

    <!-- Log Detail Modal (Simple Implementation inside LogView) -->
    <Teleport to="body">
      <transition name="modal-fade">
        <div v-if="detailLog" class="fixed inset-0 z-[120] flex items-center justify-center p-4 sm:p-6 backdrop-blur-sm transition-all duration-300">
          <div class="absolute inset-0 bg-gray-900/60" @click="detailLog = null"></div>
          <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden transform transition-all duration-300">
...
const formatFullDate = (ts) => {
  if (!ts) return '-';
  const parts = ts.split(' '); // YYYY-MM-DD HH:mm:ss
